...
namespace UI::TextWarmup
{
	namespace
	{
		...
		constexpr std::array BodyRegularSizes =
		{
			27u, 29u
		};

		// Languages that share the LocalizedRegular/LocalizedBold fonts --
		// Arabic uses its own dedicated fonts and is warmed up separately.
		constexpr std::array LocalizedLanguages =
		{
		...
		[[nodiscard]] bool WarmupFont(sf::Font& font, const sf::String& sampleText, std::span<const unsigned int> sizes,
			const TextWarmup::ProgressCallback& progress, std::size_t& combinationsTouched)
		{
			...
			for (const unsigned int size : sizes)
			{
				...
				if (progress && !progress(
					static_cast<float>(combinationsTouched) / static_cast<float>(TotalCombinationCount),
	...
	bool Run(Assets& assets, const LocalizationManager& localization, const ProgressCallback& progress)
	{
		const sf::String englishText = localization.BuildWarmupText(Language::English);
		sf::String localizedText;

		for (const Language language : LocalizedLanguages)
		{
		...
		if (!WarmupFont(assets.Fonts().Get(Config::Font::MenuSemibold), englishText, BoldSizes,
			...
			return false;
		else if (!WarmupFont(assets.Fonts().Get(Config::Font::MenuRegular), englishText, RegularSizes,
			progress, combinationsTouched))
			return false;
		else if (!WarmupFont(assets.Fonts().Get(Config::Font::BodyRegular), englishText, BodyRegularSizes,
			progress, combinationsTouched))
			return false;
		...
		if (!WarmupFont(assets.Fonts().Get(Config::Font::LocalizedBold), localizedText, BoldSizes,
			progress, combinationsTouched))
			return false;
		else if (!WarmupFont(assets.Fonts().Get(Config::Font::LocalizedRegular), localizedText, RegularSizes,
			progress, combinationsTouched))
			return false;
		else if (!WarmupFont(assets.Fonts().Get(Config::Font::ArabicBold), arabicText, BoldSizes,
			progress, combinationsTouched))
			return false;
		else if (!WarmupFont(assets.Fonts().Get(Config::Font::ArabicRegular), arabicText, RegularSizes,
			progress, combinationsTouched))
			return false;
