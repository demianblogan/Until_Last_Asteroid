...
private:
	bool MatchesEvent(const InputBinding& inputBinding, const sf::Event& event) const
	{
		using TT = InputBinding::TriggerType;

		const InputBinding::TriggerType trigger = inputBinding.GetTriggerType();

		if (trigger == TT::WhileHeld)
			return false;

		auto function = [trigger, &event](auto inputValue) -> bool
			{
		...
		return std::visit(function, inputBinding.GetInputValue());
	}

	bool IsActive(const InputBinding& inputBinding) const
	{
		if (inputBinding.GetTriggerType() != InputBinding::TriggerType::WhileHeld)
			return false;

		auto function = [](auto inputValue) -> bool
			{
				using T = decltype(inputValue);
