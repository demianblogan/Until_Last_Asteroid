...
namespace
{
	...
	Json LoadJson(const std::filesystem::path& path)
	{
		...
		{
			throw std::runtime_error("Invalid gameplay data " + path.string() + ": " + exception.what());
		}
	}
	...
	Value Require(const Json& object, const char* key, const std::filesystem::path& path)
	{
		...
		catch (const Json::exception& exception)
		{
			throw std::runtime_error(
				"Invalid gameplay value '" + std::string(key) + "' in " + path.string() + ": " + exception.what());
		}
	}
	...
	{
		if (value <= 0.f)
			throw std::runtime_error("Gameplay value '" + std::string(key) + "' must be positive in " + path.string());
	}
	...
	GameplayData::PickupKind ParsePickupKind(const std::string& value, const std::filesystem::path& path)
	{
		if (value == "health")
		...
		throw std::runtime_error("Unknown pickup type '" + value + "' in " + path.string());
	}

	std::vector<GameplayData::PickupKind> ReadPickupSequence(const Json& object,
		const char* key, const std::filesystem::path& path)
	{
		const Json& sequence = object.at(key);
		if (!sequence.is_array())
			throw std::runtime_error("Gameplay value '" + std::string(key) + "' must be an array in " + path.string());

		std::vector<GameplayData::PickupKind> result;
		result.reserve(sequence.size());

		for (const Json& value : sequence)
			result.push_back(ParsePickupKind(value.get<std::string>(), path));

		return result;
	}

	GameplayData::SpawnGroup::PickupDropConfig ReadPickupDrop(const Json& object, const std::filesystem::path& path)
	{
		GameplayData::SpawnGroup::PickupDropConfig result;
		result.chance = object.contains("chance") ? Require<float>(object, "chance", path) : 1.f;
		if (result.chance < 0.f || result.chance > 1.f)
			throw std::runtime_error("Pickup drop chance must be in [0, 1] in " + path.string());

		const bool hasType = object.contains("type");
		const bool hasPool = object.contains("pool");
		if (hasType == hasPool)
			throw std::runtime_error("Pickup drop must define exactly one of 'type' or 'pool' in " + path.string());

		if (hasType)
		{
			result.pool.push_back({ ParsePickupKind(Require<std::string>(object, "type", path), path), 1.f });
			return result;
		}

		const Json& pool = object.at("pool");
		if (!pool.is_array() || pool.empty())
			throw std::runtime_error("Pickup drop pool must be a non-empty array in " + path.string());

		for (const Json& entry : pool)
		{
			GameplayData::SpawnGroup::PickupDropConfig::WeightedPickup pickup;
			pickup.kind = ParsePickupKind(Require<std::string>(entry, "type", path), path);
	...
	std::vector<Collision::LocalCircle> ReadCollisionCircles(const Json& object, const std::filesystem::path& path)
	{
		...
		{
			throw std::runtime_error(
				"Gameplay value 'collision_circles' must contain between one and eight circles in " + path.string());
		}
		...
		for (const Json& circle : circles)
		{
			...
			if (!std::isfinite(value.offset.x) || !std::isfinite(value.offset.y))
				throw std::runtime_error("Collision-circle offsets must be finite in " + path.string());
	...
	GameplayData::ProjectileConfig ReadProjectile(const Json& object, const std::filesystem::path& path)
	{
	...
	GameplayData::BurstConfig ReadBurst(const Json& object, const std::filesystem::path& path)
	{
		...
		{
			throw std::runtime_error("Gameplay effect maximum values must not be below their minimums in " + path.string());
		}

		return result;
	}

	GameplayData::CameraShakeConfig ReadCameraShake(const Json& object, const std::filesystem::path& path)
	{
	...
	GameplayData::ParticleRangeConfig ReadRange(const Json& object, const char* key,
		GameplayData::ParticleRangeConfig fallback, const std::filesystem::path& path)
	{
	...
	std::array<int, 4> ReadColor(const Json& object, const char* key,
		std::array<int, 4> fallback, const std::filesystem::path& path)
	{
		if (!object.contains(key))
			return fallback;
	...
	GameplayData::ParticlePresetConfig ReadParticlePreset(const Json& object, const std::filesystem::path& path)
	{
	...
	GameplayData::LevelConfig::PostProcessConfig ReadPostProcess(const Json& object, const std::filesystem::path& path)
	{
		...
		if (!tint.is_array() || tint.size() != result.tint.size())
			throw std::runtime_error("Gameplay value 'tint' must contain exactly three values in " + path.string());
...
GameplayData::GameplayData(const std::filesystem::path& directory)
{
	...
	boss.maximumHealth = Require<int>(bossJson, "maximum_health", bossPath);
	boss.contactDamage = Require<int>(bossJson, "contact_damage", bossPath);
	boss.outerRingEndHealthRatio = Require<float>(bossJson, "outer_ring_end_health_ratio", bossPath);
	boss.firstShieldHealthRatio = Require<float>(bossJson, "first_shield_health_ratio", bossPath);
	boss.secondShieldHealthRatio = Require<float>(bossJson, "second_shield_health_ratio", bossPath);
	...
	boss.cannonStaggerInterval = Require<float>(bossJson, "cannon_stagger_interval", bossPath);
	boss.shieldCycleDuration = Require<float>(bossJson, "shield_cycle_duration", bossPath);
	boss.shieldWarningDuration = Require<float>(bossJson, "shield_warning_duration", bossPath);
	boss.shieldWarningBlinkInterval = Require<float>(bossJson, "shield_warning_blink_interval", bossPath);
	boss.shieldEnemyClearance = Require<float>(bossJson, "shield_enemy_clearance", bossPath);
	boss.kamikazeSpawnInterval = Require<float>(bossJson, "kamikaze_spawn_interval", bossPath);
	boss.shooterSpawnInterval = Require<float>(bossJson, "shooter_spawn_interval", bossPath);
	boss.outerRingDestructionDuration = Require<float>(bossJson, "outer_ring_destruction_duration", bossPath);
	boss.outerRingExplosionInterval = Require<float>(bossJson, "outer_ring_explosion_interval", bossPath);
	boss.diamondRotationSpeedDegrees = Require<float>(bossJson, "diamond_rotation_speed_degrees", bossPath);
	boss.portalOrbitRadius = Require<float>(bossJson, "portal_orbit_radius", bossPath);
	boss.portalCollisionRadius = Require<float>(bossJson, "portal_collision_radius", bossPath);
	boss.portalHealth = Require<int>(bossJson, "portal_health", bossPath);
	boss.portalSpawnIntervalPerAlive = Require<float>(bossJson, "portal_spawn_interval_per_alive", bossPath);
	boss.edgeShooterSpawnInterval = Require<float>(bossJson, "edge_shooter_spawn_interval", bossPath);
	...
	boss.coreCollisionRadius = Require<float>(bossJson, "core_collision_radius", bossPath);
	boss.coreBeamWidth = Require<float>(bossJson, "core_beam_width", bossPath);
	boss.coreBeamDamage = Require<int>(bossJson, "core_beam_damage", bossPath);
	boss.coreBeamSpeeds = Require<std::array<float, 3>>(bossJson, "core_beam_speeds", bossPath);
	boss.coreAsteroidSpawnInterval = Require<float>(bossJson, "core_asteroid_spawn_interval", bossPath);
	boss.coreTurretInset = Require<float>(bossJson, "core_turret_inset", bossPath);
	boss.coreStationInset = Require<float>(bossJson, "core_station_inset", bossPath);
	boss.phaseOneBonusDrops = ReadPickupSequence(bossJson, "phase_one_bonus_drops", bossPath);
	boss.phaseTwoBonusDrops = ReadPickupSequence(bossJson, "phase_two_bonus_drops", bossPath);
	boss.phaseThreeBonusDrops = ReadPickupSequence(bossJson, "phase_three_bonus_drops", bossPath);

	RequirePositive(static_cast<float>(boss.maximumHealth), "maximum_health", bossPath);
	RequirePositive(static_cast<float>(boss.contactDamage), "contact_damage", bossPath);
	RequirePositive(boss.outerRingEndHealthRatio, "outer_ring_end_health_ratio", bossPath);

	if (boss.outerRingEndHealthRatio >= 1.f)
		throw std::runtime_error("Boss outer-ring end health ratio must be below 1 in " + bossPath.string());

	if (!(boss.outerRingEndHealthRatio < boss.secondShieldHealthRatio &&
		boss.secondShieldHealthRatio < boss.firstShieldHealthRatio &&
		boss.firstShieldHealthRatio < 1.f))
	{
		throw std::runtime_error(
			"Boss Phase 1 health thresholds must descend from 1 to the outer-ring end ratio in " + bossPath.string());
	}
	...
	if (boss.cannonCount != 6)
		throw std::runtime_error("Boss must have exactly six cannons in " + bossPath.string());

	RequirePositive(boss.cannonOrbitRadius, "cannon_orbit_radius", bossPath);
	RequirePositive(boss.cannonFireInterval, "cannon_fire_interval", bossPath);
	RequireNonNegative(boss.cannonStaggerInterval, "cannon_stagger_interval", bossPath);

	if (boss.cannonStaggerInterval * static_cast<float>(boss.cannonCount - 1) >= boss.cannonFireInterval)
	{
		throw std::runtime_error("Boss cannon stagger sequence must fit inside one fire interval in " + bossPath.string());
	}

	RequirePositive(boss.shieldCycleDuration, "shield_cycle_duration", bossPath);
	RequirePositive(boss.shieldWarningDuration, "shield_warning_duration", bossPath);
	RequirePositive(boss.shieldWarningBlinkInterval, "shield_warning_blink_interval", bossPath);
	RequireNonNegative(boss.shieldEnemyClearance, "shield_enemy_clearance", bossPath);
	RequirePositive(boss.kamikazeSpawnInterval, "kamikaze_spawn_interval", bossPath);
	RequirePositive(boss.shooterSpawnInterval, "shooter_spawn_interval", bossPath);
	RequirePositive(boss.outerRingDestructionDuration, "outer_ring_destruction_duration", bossPath);
	RequirePositive(boss.outerRingExplosionInterval, "outer_ring_explosion_interval", bossPath);
	RequirePositive(boss.diamondRotationSpeedDegrees, "diamond_rotation_speed_degrees", bossPath);
	RequirePositive(boss.portalOrbitRadius, "portal_orbit_radius", bossPath);
	RequirePositive(boss.portalCollisionRadius, "portal_collision_radius", bossPath);
	RequirePositive(static_cast<float>(boss.portalHealth), "portal_health", bossPath);
	RequirePositive(boss.portalSpawnIntervalPerAlive, "portal_spawn_interval_per_alive", bossPath);
	RequirePositive(boss.edgeShooterSpawnInterval, "edge_shooter_spawn_interval", bossPath);
	...
	RequirePositive(boss.coreCollisionRadius, "core_collision_radius", bossPath);
	RequirePositive(boss.coreBeamWidth, "core_beam_width", bossPath);
	RequirePositive(static_cast<float>(boss.coreBeamDamage), "core_beam_damage", bossPath);

	for (const float speed : boss.coreBeamSpeeds)
		RequirePositive(speed, "core_beam_speeds", bossPath);

	if (!(boss.coreBeamSpeeds[0] < boss.coreBeamSpeeds[1] && boss.coreBeamSpeeds[1] < boss.coreBeamSpeeds[2]))
		throw std::runtime_error("Boss core beam speeds must increase in " + bossPath.string());

	RequirePositive(boss.coreAsteroidSpawnInterval, "core_asteroid_spawn_interval", bossPath);
	RequirePositive(boss.coreTurretInset, "core_turret_inset", bossPath);
	RequirePositive(boss.coreStationInset, "core_station_inset", bossPath);

	if (boss.phaseOneBonusDrops.size() != 2u ||
		boss.phaseTwoBonusDrops.size() != 4u ||
		boss.phaseThreeBonusDrops.size() != 2u)
	{
		throw std::runtime_error(
	...
	{
		throw std::runtime_error("Invalid engine emitters in " + playerPath.string() + ": " + exception.what());
	}
	try
	{
	...
	{
		throw std::runtime_error("Invalid muzzle emitter in " + playerPath.string() + ": " + exception.what());
	}
	...
	for (const auto& [name, kind] : enemyNames)
	{
		try
		{
			...
			if (kind == EnemyKind::Kamikaze && !enemyJson.contains("rotation_speed"))
				throw std::runtime_error("Enemy 'kamikaze' requires 'rotation_speed' in " + enemiesPath.string());

			if ((kind == EnemyKind::Shooter || kind == EnemyKind::Spinner || kind == EnemyKind::MissileCarrier) &&
				!enemyJson.contains("weapon_emitters"))
				throw std::runtime_error("Armed enemy requires 'weapon_emitters' in " + enemiesPath.string());
			...
			if (kind == EnemyKind::Shooter || kind == EnemyKind::MissileCarrier || kind == EnemyKind::ReflectorGunship)
			{
				if (!enemyJson.contains("rotation_speed"))
					throw std::runtime_error("Aiming enemy requires 'rotation_speed' in " + enemiesPath.string());
			...
			if (kind == EnemyKind::Shooter && config.weaponEmitters.size() != 2)
				throw std::runtime_error("Enemy 'shooter' requires exactly two weapon emitters in " + enemiesPath.string());

			if (kind == EnemyKind::Spinner)
			{
				if (config.weaponEmitters.size() != 3)
					throw std::runtime_error("Enemy 'spinner' requires exactly three weapon emitters in " + enemiesPath.string());
			...
			if (kind == EnemyKind::MissileCarrier && config.weaponEmitters.size() != 1)
				throw std::runtime_error("Enemy 'missile_carrier' requires exactly one weapon emitter in " + enemiesPath.string());

			if (kind == EnemyKind::MissileCarrier && config.engineEmitters.size() != 2)
				throw std::runtime_error("Enemy 'missile_carrier' requires exactly two engine emitters in " + enemiesPath.string());
			...
			if (kind == EnemyKind::ShooterStation)
			{
				...
				RequirePositive(config.spawnAnimationDuration, "spawn_animation_duration", enemiesPath);
			}
			if (kind == EnemyKind::ReflectorGunship)
			{
				...
				if (config.weaponEmitters.size() != 2)
					throw std::runtime_error(
						"Enemy 'reflector_gunship' requires exactly two weapon emitters in " + enemiesPath.string());
			}
	...
	try
	{
		...
		effects.smallAsteroidExplosion = ReadBurst(effectsJson.at("small_asteroid_explosion"), effectsPath);
		effects.largeAsteroidExplosion = ReadBurst(effectsJson.at("large_asteroid_explosion"), effectsPath);
		...
		effects.largeExplosionShake = ReadCameraShake(effectsJson.at("large_explosion_shake"), effectsPath);
	...
	pickups.healthRestorePercentage = Require<float>(pickupsJson, "health_restore_percentage", pickupsPath);
	...
	pickups.homingBulletsDuration = Require<float>(pickupsJson, "homing_bullets_duration", pickupsPath);
	pickups.homingConeDegrees = Require<float>(pickupsJson, "homing_cone_degrees", pickupsPath);
	pickups.homingTurnSpeedDegrees = Require<float>(pickupsJson, "homing_turn_speed_degrees", pickupsPath);
	pickups.laserDuration = Require<float>(pickupsJson, "laser_duration", pickupsPath);
	pickups.laserDamageInterval = Require<float>(pickupsJson, "laser_damage_interval", pickupsPath);
	pickups.laserWidth = Require<float>(pickupsJson, "laser_width", pickupsPath);
	pickups.tripleShotDuration = Require<float>(pickupsJson, "triple_shot_duration", pickupsPath);
	pickups.tripleShotAngleDegrees = Require<float>(pickupsJson, "triple_shot_angle_degrees", pickupsPath);
	pickups.timeSlowdownDuration = Require<float>(pickupsJson, "time_slowdown_duration", pickupsPath);
	pickups.timeSlowdownWorldScale = Require<float>(pickupsJson, "time_slowdown_world_scale", pickupsPath);
	pickups.timeSlowdownAudioPitch = Require<float>(pickupsJson, "time_slowdown_audio_pitch", pickupsPath);
	pickups.helperBotShotInterval = Require<float>(pickupsJson, "helper_bot_shot_interval", pickupsPath);
	pickups.helperBotTurnSpeedDegrees = Require<float>(pickupsJson, "helper_bot_turn_speed_degrees", pickupsPath);
	pickups.helperBotOrbitRadius = Require<float>(pickupsJson, "helper_bot_orbit_radius", pickupsPath);
	pickups.helperBotOrbitSpeedDegrees = Require<float>(pickupsJson, "helper_bot_orbit_speed_degrees", pickupsPath);
	pickups.helperBotVisualScale = Require<float>(pickupsJson, "helper_bot_visual_scale", pickupsPath);
	...
	if (pickups.homingConeDegrees > 360.f)
		throw std::runtime_error("Homing cone must not exceed 360 degrees in " + pickupsPath.string());
	...
	if (pickups.tripleShotAngleDegrees >= 45.f)
		throw std::runtime_error("Triple-shot angle must be below 45 degrees in " + pickupsPath.string());
	...
	if (pickups.timeSlowdownWorldScale > 1.f)
		throw std::runtime_error("Time slowdown world scale must not exceed 1 in " + pickupsPath.string());
	...
	if (pickups.timeSlowdownAudioPitch > 1.f)
		throw std::runtime_error("Time slowdown audio pitch must not exceed 1 in " + pickupsPath.string());
	...
	parts.rotationSpeedDegrees = Require<float>(pickupsJson, "part_rotation_speed_degrees", pickupsPath);
	...
	{
		throw std::runtime_error("Invalid levels in " + levelsPath.string() + ": " + exception.what());
	}
	...
	for (const Json& levelJson : *levelArray)
	{
		...
		const std::string encounter = levelJson.value("encounter", "waves");

		if (encounter == "waves")
			level.encounter = EncounterKind::Waves;
		else if (encounter == "boss")
			level.encounter = EncounterKind::Boss;
		else
			throw std::runtime_error("Unknown encounter type '" + encounter + "' in " + levelsPath.string());

		level.background = Require<std::string>(levelJson, "background", levelsPath);
		level.backgroundBrightness = Require<float>(levelJson, "background_brightness", levelsPath);
		level.targetAccuracyPercent = Require<float>(levelJson, "target_accuracy_percent", levelsPath);
		...
		if (level.targetAccuracyPercent > 100.f)
			throw std::runtime_error("Target accuracy must not exceed 100 percent in " + levelsPath.string());

		if (level.backgroundBrightness > 1.f)
			throw std::runtime_error("Level background brightness must not exceed 1 in " + levelsPath.string());
		...
		try
		{
			for (const Json& waveJson : levelJson.at("waves"))
			{
				...
				for (const Json& spawnJson : waveJson.at("initial_spawns"))
				{
					...
					if (spawn.count <= 0)
						throw std::runtime_error("Wave initial spawn count must be positive in " + levelsPath.string());
				...
				for (const Json& spawnJson : waveJson.at("scheduled_spawns"))
				{
					...
					if (spawn.count <= 0)
						throw std::runtime_error("Scheduled spawn count must be positive in " + levelsPath.string());
			...
			if (level.encounter == EncounterKind::Waves && level.waves.size() != 3u)
				throw std::runtime_error("Wave encounters must contain exactly three waves in " + levelsPath.string());

			if (level.encounter == EncounterKind::Boss &&
				(level.number != 10 || !level.waves.empty()))
			{
				throw std::runtime_error(
					"Only Level 10 may be a boss encounter, and it must not contain waves in " + levelsPath.string());
			}
		}
		catch (const Json::exception& exception)
		{
			throw std::runtime_error(
				"Invalid level " + std::to_string(level.number) + " in " + levelsPath.string() + ": " + exception.what());
		}
...
const GameplayData::BossConfig& GameplayData::GetBoss() const noexcept
{
	return boss;
}
