...
namespace
{
	...
	std::vector<GameplayData::PickupKind> ReadPickupSequence(
		const Json& object,
		const char* key,
		const std::filesystem::path& path)
	{
		...
		if (!sequence.is_array())
			throw std::runtime_error(
				"Gameplay value '" + std::string(key) + "' must be an array in " + path.string());
...
GameplayData::GameplayData(const std::filesystem::path& directory)
{
	...
	boss.outerRingEndHealthRatio = Require<float>(
		bossJson, "outer_ring_end_health_ratio", bossPath);
	boss.firstShieldHealthRatio = Require<float>(
		bossJson, "first_shield_health_ratio", bossPath);
	boss.secondShieldHealthRatio = Require<float>(
		bossJson, "second_shield_health_ratio", bossPath);
	boss.outerRingRotationSpeedDegrees = Require<float>(
		bossJson, "outer_ring_rotation_speed_degrees", bossPath);
	boss.outerRingInnerRadius = Require<float>(
		bossJson, "outer_ring_inner_radius", bossPath);
	boss.outerRingOuterRadius = Require<float>(
		bossJson, "outer_ring_outer_radius", bossPath);
	boss.cannonCount = Require<int>(bossJson, "cannon_count", bossPath);
	boss.cannonOrbitRadius = Require<float>(
		bossJson, "cannon_orbit_radius", bossPath);
	boss.cannonFireInterval = Require<float>(
		bossJson, "cannon_fire_interval", bossPath);
	boss.cannonStaggerInterval = Require<float>(
		bossJson, "cannon_stagger_interval", bossPath);
	boss.shieldCycleDuration = Require<float>(
		bossJson, "shield_cycle_duration", bossPath);
	boss.shieldWarningDuration = Require<float>(
		bossJson, "shield_warning_duration", bossPath);
	boss.shieldWarningBlinkInterval = Require<float>(
		bossJson, "shield_warning_blink_interval", bossPath);
	boss.shieldEnemyClearance = Require<float>(
		bossJson, "shield_enemy_clearance", bossPath);
	boss.kamikazeSpawnInterval = Require<float>(
		bossJson, "kamikaze_spawn_interval", bossPath);
	boss.shooterSpawnInterval = Require<float>(
		bossJson, "shooter_spawn_interval", bossPath);
	boss.outerRingDestructionDuration = Require<float>(
		bossJson, "outer_ring_destruction_duration", bossPath);
	boss.outerRingExplosionInterval = Require<float>(
		bossJson, "outer_ring_explosion_interval", bossPath);
	boss.diamondRotationSpeedDegrees = Require<float>(
		bossJson, "diamond_rotation_speed_degrees", bossPath);
	boss.portalOrbitRadius = Require<float>(bossJson, "portal_orbit_radius", bossPath);
	boss.portalCollisionRadius = Require<float>(
		bossJson, "portal_collision_radius", bossPath);
	boss.portalHealth = Require<int>(bossJson, "portal_health", bossPath);
	boss.portalSpawnIntervalPerAlive = Require<float>(
		bossJson, "portal_spawn_interval_per_alive", bossPath);
	boss.edgeShooterSpawnInterval = Require<float>(
		bossJson, "edge_shooter_spawn_interval", bossPath);
	boss.innerShieldDuration = Require<float>(bossJson, "inner_shield_duration", bossPath);
	boss.diamondFrameVertexRadius = Require<float>(
		bossJson, "diamond_frame_vertex_radius", bossPath);
	boss.diamondFrameHalfThickness = Require<float>(
		bossJson, "diamond_frame_half_thickness", bossPath);
	boss.diamondDestructionDuration = Require<float>(
		bossJson, "diamond_destruction_duration", bossPath);
	boss.diamondExplosionInterval = Require<float>(
		bossJson, "diamond_explosion_interval", bossPath);
	...
	boss.coreBeamSpeeds = Require<std::array<float, 3>>(
		bossJson, "core_beam_speeds", bossPath);
	boss.coreAsteroidSpawnInterval = Require<float>(
		bossJson, "core_asteroid_spawn_interval", bossPath);
	...
	boss.phaseOneBonusDrops = ReadPickupSequence(
		bossJson, "phase_one_bonus_drops", bossPath);
	boss.phaseTwoBonusDrops = ReadPickupSequence(
		bossJson, "phase_two_bonus_drops", bossPath);
	boss.phaseThreeBonusDrops = ReadPickupSequence(
		bossJson, "phase_three_bonus_drops", bossPath);
	...
	RequirePositive(boss.outerRingEndHealthRatio,
		"outer_ring_end_health_ratio", bossPath);
	if (boss.outerRingEndHealthRatio >= 1.f)
		throw std::runtime_error(
			"Boss outer-ring end health ratio must be below 1 in " + bossPath.string());
	...
	{
		throw std::runtime_error(
			"Boss Phase 1 health thresholds must descend from 1 to the outer-ring end ratio in " +
			bossPath.string());
	}
	RequirePositive(boss.outerRingRotationSpeedDegrees,
		"outer_ring_rotation_speed_degrees", bossPath);
	...
	if (boss.outerRingInnerRadius >= boss.outerRingOuterRadius)
		throw std::runtime_error(
			"Boss outer-ring inner radius must be below its outer radius in " +
			bossPath.string());
	...
	RequireNonNegative(boss.cannonStaggerInterval,
		"cannon_stagger_interval", bossPath);
	if (boss.cannonStaggerInterval * static_cast<float>(boss.cannonCount - 1) >=
		boss.cannonFireInterval)
	{
		throw std::runtime_error(
			"Boss cannon stagger sequence must fit inside one fire interval in " +
			bossPath.string());
	}
	...
	RequirePositive(boss.shieldWarningBlinkInterval,
		"shield_warning_blink_interval", bossPath);
	...
	RequirePositive(boss.outerRingDestructionDuration,
		"outer_ring_destruction_duration", bossPath);
	RequirePositive(boss.outerRingExplosionInterval,
		"outer_ring_explosion_interval", bossPath);
	RequirePositive(boss.diamondRotationSpeedDegrees,
		"diamond_rotation_speed_degrees", bossPath);
	...
	RequirePositive(boss.portalSpawnIntervalPerAlive,
		"portal_spawn_interval_per_alive", bossPath);
	RequirePositive(boss.edgeShooterSpawnInterval,
		"edge_shooter_spawn_interval", bossPath);
	RequirePositive(boss.innerShieldDuration, "inner_shield_duration", bossPath);
	RequirePositive(boss.diamondFrameVertexRadius,
		"diamond_frame_vertex_radius", bossPath);
	RequirePositive(boss.diamondFrameHalfThickness,
		"diamond_frame_half_thickness", bossPath);
	RequirePositive(boss.diamondDestructionDuration,
		"diamond_destruction_duration", bossPath);
	RequirePositive(boss.diamondExplosionInterval,
		"diamond_explosion_interval", bossPath);
	...
	if (!(boss.coreBeamSpeeds[0] < boss.coreBeamSpeeds[1] &&
		boss.coreBeamSpeeds[1] < boss.coreBeamSpeeds[2]))
		throw std::runtime_error(
			"Boss core beam speeds must increase in " + bossPath.string());
	RequirePositive(boss.coreAsteroidSpawnInterval,
		"core_asteroid_spawn_interval", bossPath);
	...
	if (boss.phaseOneBonusDrops.size() != 2u ||
		...
		throw std::runtime_error(
			"Boss bonus-drop phases must contain exactly two, four, and two pickups in " +
			bossPath.string());
    ...
    for (const Json& levelJson : *levelArray)
    {
		...
		if (encounter == "waves")
			...
			throw std::runtime_error(
				"Unknown encounter type '" + encounter + "' in " + levelsPath.string());
        ...
        try
        {
			...
			if (level.encounter == EncounterKind::Waves && level.waves.size() != 3u)
				throw std::runtime_error(
					"Wave encounters must contain exactly three waves in " +
					levelsPath.string());
			if (level.encounter == EncounterKind::Boss &&
				(level.number != 10 || !level.waves.empty()))
			{
				throw std::runtime_error(
					"Only Level 10 may be a boss encounter, and it must not contain waves in " +
					levelsPath.string());
			}
        }
        catch (const Json::exception& exception)
