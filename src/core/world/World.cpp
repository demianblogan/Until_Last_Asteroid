...
namespace
{
	// Guards FindHomingTarget's direction-dot test below against a target that
	// coincides (or nearly does) with position -- a zero offset has no
	// meaningful direction to dot against.
	constexpr float CoincidentTargetEpsilonSquared = 0.0001f;

	void DrawPickupAura(sf::RenderTarget& target, const Pickup& pickup, float visualTime, sf::RenderStates states)
	{
		sf::Color color;
	...
	void DrawShieldOverlay(sf::RenderTarget& target, const Player& player, float visualTime, float ratio,
		float hitFlashRatio, sf::RenderStates states)
	{
		...
		Rendering::DrawEnergyShield(target, player.GetPosition(), player.GetCollisionRadius() * 1.62f, pulse,
			{ 20, 205, 235 }, { 70, 240, 255 }, { 45, 225, 255 }, 24.f, states);
	}

	void DrawFeatheredBeam(sf::RenderTarget& target, const sf::Vector2f& start, const sf::Vector2f& end,
		float beamWidth, float pulse, sf::Color coreColor, sf::Color glowColor, sf::RenderStates states)
	{
	...
	void DrawLaserBeam(sf::RenderTarget& target, const LaserTurret& turret, sf::RenderStates states)
	{
		...
		DrawFeatheredBeam(target, start, turret.GetBeamEnd(), turret.GetBeamWidth(), pulse,
		...
		for (int index = 0; index < MarkerCount; index++)
		{
			...
			diamond.setPosition(start + direction * (travel * length) + perpendicular * offset);
			diamond.setFillColor(sf::Color(255, 210, 125, static_cast<std::uint8_t>(135.f + depth * 120.f)));

			target.draw(diamond, states);
		}
	}

	void DrawPlayerLaser(sf::RenderTarget& target, const Player& player, float beamWidth, sf::RenderStates states)
	{
		...
		DrawFeatheredBeam(target, start, player.GetLaserEndPosition(), beamWidth, pulse,
		...
		for (int index = 0; index < MarkerCount; index++)
		{
			...
			spark.setPosition(start + direction * (travel * length) + perpendicular * (side * beamWidth * 0.7f));
	...
	void DrawStationEffects(sf::RenderTarget& target, const ShooterStation& station, sf::RenderStates states)
	{
		states.blendMode = sf::BlendAdd;
		...
		if (charge > 0.f)
		{
			...
			portal.setFillColor(sf::Color(255, 28, 18, static_cast<std::uint8_t>(70.f * charge)));
			portal.setOutlineColor(sf::Color(255, 120, 55, static_cast<std::uint8_t>(210.f * charge)));
		...
		Rendering::DrawEnergyShield(target, station.GetPosition(), station.GetShieldRadius(), station.GetShieldPulse(),
			{ 220, 28, 38 }, { 255, 78, 62 }, { 255, 40, 35 }, 64.f, states);
	}
}

World::World(unsigned int width, unsigned int height,
	Assets& assets, AudioManager& audio, GameplaySession& session, GamepadManager& gamepadManager,
...
void World::Update(float deltaTime, float worldTimeScale)
{
	...
	for (auto& entity : entities)
	{
		...
		const float entityDeltaTime = usesPlayerTime ? deltaTime : deltaTime * worldTimeScale;
...
void World::SpawnPlayer(Assets& playerAssets, InputHandler<Config::PlayerAction>& input, sf::RenderWindow& window)
{
	if (player != nullptr)
		return;
...
Player* World::GetPlayer() const noexcept
{
	return player;
}
...
{
	if (isHelperPickupSpawned || session.IsHelperBotActive())
		return false;

	auto pickup = std::make_unique<Pickup>(assets, *this, Pickup::Kind::HelperBot);
	pickup->SetPosition(pos);
	Spawn(std::move(pickup));

	isHelperPickupSpawned = true;
...
{
	if (isHelperBotSpawned || !session.IsHelperBotActive())
		return;

	isHelperBotSpawned = true;

	Spawn(std::make_unique<HelperBot>(assets, *this));
}

void World::SpawnSaucerShot(const sf::Vector2f& pos, const sf::Vector2f& target,
	GameplayData::ProjectileKind projectileKind, bool needToPlaySound)
{
	Spawn(std::make_unique<SaucerShot>(assets, *this, pos, target, projectileKind, needToPlaySound));
}
...
void World::DamageEnemiesWithPlayerLaser(const sf::Vector2f& start, const sf::Vector2f& end,
	float laserWidth, int damage, std::uint64_t attackID)
{
	// Guards the hit-test below against a degenerate (near-zero-length) laser
	// segment, which would have no meaningful direction to report on a hit.
	constexpr float DegenerateSegmentEpsilonSquared = 0.001f;
	const sf::Vector2f segment{ end - start };
	if (segment.x * segment.x + segment.y * segment.y <= DegenerateSegmentEpsilonSquared || damage <= 0)
		return;

	playerLaserDamageEvent = PlayerLaserDamageEvent{ start, end, laserWidth, damage, attackID };
	...
	for (const auto& entity : entities)
	{
		...
		effectEvents.Add({ Rendering::EffectEventType::ShipHit,	*impactPosition, VectorMath::Normalize(segment),
			std::clamp(entity->GetCollisionRadius() / 45.f, 0.55f, 1.2f) });

		if (type == Entity::Type::EnemyMissile)
		{
			static_cast<void>(static_cast<HomingMissile&>(*entity).TakeDamage(damage));
			continue;
		}
...
void World::DamagePlayerWithBeam(const sf::Vector2f& start, const sf::Vector2f& end, float beamWidth, int damage)
{
	...
	if (player->TakeDamage(damage))
	{
		if (player->DidLastDamageReachHealth())
			effectEvents.Add({ Rendering::EffectEventType::PlayerHit, *closest, VectorMath::Normalize(segment), 1.15f });

		sound.AddSound(Config::Sound::MetalHit, 0.85f);
		VibrationProfiles::Apply(gamepadHaptics, VibrationProfiles::Light);

		HandleGameOverIfPlayerDied(*player);
	}
}

void World::ExplodeEnemyMissile(const sf::Vector2f& position, float radius, int damage, float impulse)
{
	for (auto& entityPointer : entities)
	{
		...
		const sf::Vector2f direction = VectorMath::Normalize(offset);
		...
		if (entity.GetType() == Entity::Type::Player)
		{
			...
			if (damageAccepted)
			{
				if (targetPlayer.DidLastDamageReachHealth())
					effectEvents.Add({ Rendering::EffectEventType::PlayerHit, targetPlayer.GetPosition(), direction, 1.25f });

				targetPlayer.ApplyImpulse(direction * impulse);
				VibrationProfiles::Apply(gamepadHaptics, VibrationProfiles::Light);
			}

			HandleGameOverIfPlayerDied(targetPlayer);

			continue;
		}

		if (entity.GetType() != Entity::Type::Enemy && entity.GetType() != Entity::Type::Asteroid)
		{
			continue;
		}
		...
		const bool isEnemyKilled = enemy.TakeDamage(damage);
...
WorldSoundSystem& World::Sound() noexcept
{
	return sound;
}

WorldEffectEventQueue& World::Effects() noexcept
{
	return effectEvents;
}

Haptics::GamepadHaptics& World::Haptics() noexcept
{
	return gamepadHaptics;
}

void World::HandleGameOverIfPlayerDied(Player& possiblyDeadPlayer)
{
	if (possiblyDeadPlayer.IsAlive())
		return;

	session.SetGameOver();
	VibrationProfiles::Apply(gamepadHaptics, VibrationProfiles::Death);
}
...
std::optional<World::PlayerLaserDamageEvent> World::ConsumePlayerLaserDamageEvent() noexcept
{
	return std::exchange(playerLaserDamageEvent, std::nullopt);
}

std::vector<World::PlayerProjectileImpact> World::ConsumePlayerProjectilesInCircle(sf::Vector2f center, float radius)
{
	...
	for (const auto& entity : entities)
	{
		...
		const auto direction =
			ProjectileGeometry::TryCircleImpactDirection(center, radius, entity->GetPosition(), entity->GetCollisionRadius());
		...
		entity->Destroy();
		effectEvents.Add({ Rendering::EffectEventType::ShipHit, impacts.back().position, *direction, 1.25f });
	...
	sf::Vector2f center, float innerRadius, float outerRadius)
{
	...
	for (const auto& entity : entities)
	{
		...
		const auto direction = ProjectileGeometry::TryAnnulusImpactDirection(center, innerRadius, outerRadius,
		...
		entity->Destroy();
		effectEvents.Add({ Rendering::EffectEventType::ShipHit, impacts.back().position, *direction, 1.15f });
...
void World::ConsumePlayerProjectilesInDiamondFrame(sf::Vector2f center, float rotationDegrees,
	float vertexRadius, float halfThickness)
{
	for (const auto& entity : entities)
	{
		...
		const auto direction =
			ProjectileGeometry::TryDiamondFrameImpactDirection(center, rotationDegrees, vertexRadius, halfThickness,
				entity->GetPosition(), entity->GetCollisionRadius());
		...
		entity->Destroy();
		effectEvents.Add({ Rendering::EffectEventType::ShipHit, entity->GetPosition(), *direction, 0.9f });
...
bool World::DamagePlayerFromBoss(int damage, sf::Vector2f sourcePosition)
{
	...
	const sf::Vector2f direction = VectorMath::Normalize(player->GetPosition() - sourcePosition);

	if (player->DidLastDamageReachHealth())
	{
		effectEvents.Add({ Rendering::EffectEventType::PlayerHit, player->GetPosition(), direction, 1.2f });
	}
...
void World::KeepPlayerOutsideCircle(sf::Vector2f center, float radius, int contactDamage)
{
	if (player == nullptr || !player->IsAlive())
		return;
	...
	const sf::Vector2f direction = VectorMath::Normalize(offset, { 0.f, 1.f });
...
void World::KeepEnemiesOutsideCircle(sf::Vector2f center, float radius, float clearance)
{
	const auto keepOutside = [&](auto& list)
		{
			for (auto& entity : list)
			{
				...
				const float minimumDistance = radius + clearance + entity->GetCollisionRadius();
	...
	const auto countIn = [](const auto& list)
		{
			return static_cast<std::size_t>(std::count_if(
				...
				{
					return entity->IsAlive() && dynamic_cast<const LaserTurret*>(entity.get()) != nullptr;
				}));
		};
...
bool World::DestroyNextBossVictoryTarget()
{
	const auto destroyNextIn{ [this](auto& list)
	{
		for (const auto& entity : list)
		{
			...
			if (type != Entity::Type::Enemy && type != Entity::Type::Asteroid && type != Entity::Type::EnemyMissile)
			{
				continue;
			}
			...
			{
				effectEvents.Add({ Rendering::EffectEventType::ShipExplosion, entity->GetPosition(), {}, 0.7f });
			}
...
void World::ClearBossVictoryPickupsAndCompanions()
{
	const auto remove = [](const auto& entity)
		{
			const Entity::Type type = entity->GetType();
			return type == Entity::Type::Pickup || type == Entity::Type::Part || type == Entity::Type::Companion;
		};
...
void World::ClearPickups()
{
	const auto isPickup = [](const auto& entity)
		{
			return entity->GetType() == Entity::Type::Pickup || entity->GetType() == Entity::Type::Part;
		};
...
void World::ConfigureCampaignPickupSequence(const std::vector<GameplayData::PickupKind>& sequence)
{
...
const Entity* World::FindHomingTarget(const sf::Vector2f& position,
	const sf::Vector2f& direction, float minimumDirectionDot) const noexcept
{
	...
	for (const auto& entity : entities)
	{
		...
		const float directionDot = (offset.x * normalizedDirection.x + offset.y * normalizedDirection.y) * inverseDistance;
...
WorldBossHomingTargets& World::BossHomingTargets() noexcept
{
	return bossHomingTargets;
}
...
GameplaySession& World::GetSession() noexcept
{
	return session;
}

const World::Statistics& World::GetStatistics() const noexcept
{
	return statisticsTracker.Get();
}
...
void World::Clear()
{
	...
	isHelperPickupSpawned = false;
	isHelperBotSpawned = false;
	...
	const auto cellIndex = [columns, rows](int cx, int cy) -> std::size_t
		{
			...
			return static_cast<std::size_t>(wrappedY) * static_cast<std::size_t>(columns) + static_cast<std::size_t>(wrappedX);
		};
		...
		for (int dy = -1; dy <= 1; dy++)
		{
			for (int dx = -1; dx <= 1; dx++)
			{
				for (const std::size_t j : grid[cellIndex(cx + dx, cy + dy)])
				{
					...
					if (first.IsAlive() && second.IsAlive() && first.IsCollidingWith(second) && second.IsCollidingWith(first))
					{
...
void World::HandleCollisionPair(Entity& first, Entity& second)
{
	...
	{
		Part& part = static_cast<Part&>(first.GetType() == Entity::Type::Part ? first : second);
	...
	{
		Pickup& pickup = static_cast<Pickup&>(first.GetType() == Entity::Type::Pickup ? first : second);
	...
	if (first.GetType() == Entity::Type::EnemyMissile || second.GetType() == Entity::Type::EnemyMissile)
	{
		HomingMissile& missile = static_cast<HomingMissile&>(first.GetType() == Entity::Type::EnemyMissile ? first : second);
		Entity& other = first.GetType() == Entity::Type::EnemyMissile ? second : first;

		if (other.GetType() == Entity::Type::Projectile_Player || other.GetType() == Entity::Type::Projectile_Ally)
		{
			Shot& shot = static_cast<Shot&>(other);

			RegisterPlayerAttackHit(shot.GetPlayerAttackID());
			effectEvents.Add(
				{ Rendering::EffectEventType::ShipHit, missile.GetPosition(), VectorMath::Normalize(shot.GetVelocity()), 0.55f });
	...
	auto handleFriendlyShot = [this](Shot& shot, Enemy& enemy)
		{
			RegisterPlayerAttackHit(shot.GetPlayerAttackID());
			const sf::Vector2f impactDirection{ VectorMath::Normalize(shot.GetVelocity()) };
			const sf::Vector2f impactPosition{ enemy.GetPlayerProjectileImpactPosition(shot) };
			...
			{
				const sf::Vector2f reflectedDirection{ VectorMath::Normalize(GetPlayerPosition() - impactPosition) };
			...
			const bool isEnemyKilled = enemy.TakeDamage(shot.GetDamage());
	...
	auto handleEnemyShot = [this](Shot& shot, Entity& target)
		{
			...
			if (target.GetType() == Entity::Type::Player)
			{
				...
				const bool isDamageAccepted = targetPlayer.TakeDamage(shot.GetDamage());
				...
				if (isDamageAccepted)
				{
					...
					if (targetPlayer.DidLastDamageReachHealth())
						effectEvents.Add({ Rendering::EffectEventType::PlayerHit, shot.GetPosition(), impactDirection, 1.f });

					targetPlayer.ApplyImpulse(impactDirection * shot.GetKnockback());
					VibrationProfiles::Apply(gamepadHaptics, VibrationProfiles::Light);
				}

				if (isDamageAccepted && targetPlayer.IsAlive())
					sound.AddSound(Config::Sound::MetalHit);
			...
			else
			{
				...
				static_cast<void>(enemy.TakeDamage(shot.GetDamage()));

				if (enemy.AcceptsKnockback())
					enemy.ApplyImpulse(VectorMath::Normalize(shot.GetVelocity()) * shot.GetKnockback());
			}
		};
	...
	const bool isDamageAccepted = collidedPlayer->TakeDamage(collidedEnemy->GetContactDamage());

	if (isDamageAccepted)
	{
		const sf::Vector2f impactDirection{ VectorMath::Normalize(collidedEnemy->GetPosition() - collidedPlayer->GetPosition()) };
		const sf::Vector2f impactPosition{ collidedPlayer->GetPosition() + impactDirection * collidedPlayer->GetCollisionRadius() };

		if (collidedPlayer->DidLastDamageReachHealth())
			effectEvents.Add({ Rendering::EffectEventType::PlayerHit, impactPosition, -impactDirection, 1.15f });
	...
	HandleGameOverIfPlayerDied(*collidedPlayer);
	...
	if (isDamageAccepted)
	{
		...
		if (collidedEnemy->AcceptsKnockback())
			collidedEnemy->ApplyImpulse(manifold->normal * assets.GetGameplayData().GetPlayer().collisionImpulse);
	}
}
...
void World::AwardScore(const Enemy& enemy)
{
	...
	{
		if (const Meteor* meteor = dynamic_cast<const Meteor*>(&enemy))
		{
	...
	for (std::size_t index = 0u; index < pickupKinds.size(); index++)
	{
		...
		{
			rewardExclusionZone.PushOutside(enemy.GetPosition() +
		...
		{
			auto pickup = std::make_unique<Pickup>(assets, *this, pickupKinds[index]);
	...
	if (enemy.IsPartRewardEnabled() && !enemy.GetPartDropID().empty() && !session.IsPartCollected(enemy.GetPartDropID()))
	{
...
void World::ResolveCollision(Entity& first, Entity& second, const sf::Vector2f& normal, float penetration) const
{
...
{
	sf::Shader& enemyEmissionShader = assets.GetShader(Config::Shader::EnemyEmission);
	enemyEmissionShader.setUniform("source", sf::Shader::CurrentTexture);

	sf::Shader& playerEmissionShader = assets.GetShader(Config::Shader::PlayerEmission);
	...
	const auto drawEntity = [&](const std::unique_ptr<Entity>& entity)
		{
			...
			if (entity->GetType() == Entity::Type::Enemy || entity->GetType() == Entity::Type::EnemyMissile)
			{
			...
			else if (entity->GetType() == Entity::Type::Player || entity->GetType() == Entity::Type::Companion)
			{
				sf::RenderStates emissionStates{ states };
	...
	if (player != nullptr)
	{
		DrawPlayerLaser(target, *player, assets.GetGameplayData().GetPickups().laserWidth, states);
	}
	for (const auto& entity : entities)
	{
		if (!dynamic_cast<const ShooterStation*>(entity.get()))
