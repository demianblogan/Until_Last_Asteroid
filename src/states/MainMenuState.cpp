	...
	, buttonList(context.audio, neonGlow)
{
	...
	for (std::size_t index = 0u; index < menuLabels.size(); index++)
	{
...
void MainMenuState::RefreshLocalizedLabels()
{
	...
	const Config::Font fontID = language == Language::English
		? Config::Font::MenuRegular
	...
	const sf::FloatRect fullTitleBounds = title.getLocalBounds();
	titleLeftPosition = GetContext().logicalSize.x * 0.5f -
		fullTitleBounds.size.x * 0.5f - fullTitleBounds.position.x;
	title.setOrigin({ 0.f, fullTitleBounds.position.y + fullTitleBounds.size.y * 0.5f });

	const std::vector<sf::String> labels = GetMenuLabels(GetContext().localization);
	for (std::size_t i = 0u; i < buttonList.GetButtons().size() && i < labels.size(); i++)
	{
...
void MainMenuState::ApplyAnimationState()
{
	const sf::String visibleTitle = hasRefreshedLocalizedLabels
		? GetContext().localization.GetText("main_menu.title")
		: introAnimation.GetVisibleTitle();
	...
	const float titleY = TitleStartY + (TitleEndY - TitleStartY) * introAnimation.GetTitleMoveProgress();
	...
	const std::uint8_t versionAlpha = static_cast<std::uint8_t>(frameOpacity * 175.f);
