...
void Shot::Update(float deltaTime)
{
	...
	const float velocityLength = std::sqrt(currentVelocity.x * currentVelocity.x + currentVelocity.y * currentVelocity.y);
	const sf::Vector2f direction{ velocityLength > 0.0001f ? currentVelocity / velocityLength : sf::Vector2f{ 0.f, -1.f } };

	GetWorld().Effects().Add({ GetTrailGlowType(visualKind), position, direction });
...
PlayerShot::PlayerShot(Assets& assets, World& world, const sf::Vector2f& position, float rotationDegrees,
	...
	, isHomingEnabled(world.GetSession().IsHomingBulletsActive())
{
	...
	const float angle = rotationDegrees * std::numbers::pi_v<float> / 180.f - std::numbers::pi_v<float> / 2.f;
...
void PlayerShot::AcquireHomingTarget()
{
	...
	const float halfConeRadians = config.homingConeDegrees * 0.5f * std::numbers::pi_v<float> / 180.f;

	homingTarget = GetWorld().FindHomingTarget(GetPosition(), GetVelocity(), std::cos(halfConeRadians));
	bossHomingTarget =
		GetWorld().BossHomingTargets().FindClosest(GetPosition(), GetVelocity(), std::cos(halfConeRadians));
	...
	if (homingTarget != nullptr && bossHomingTarget)
	{
		...
		{
			const sf::Vector2f entityOffset{ homingTarget->GetPosition() - GetPosition() };
			const sf::Vector2f bossOffset{ *bossPosition - GetPosition() };
			const float entityDistanceSquared = entityOffset.x * entityOffset.x + entityOffset.y * entityOffset.y;
			const float bossDistanceSquared = bossOffset.x * bossOffset.x + bossOffset.y * bossOffset.y;
...
void PlayerShot::UpdateHoming(float deltaTime)
{
	if (!GetWorld().IsEntityActive(homingTarget) &&
		(!bossHomingTarget || !GetWorld().BossHomingTargets().Get(*bossHomingTarget)))
	{
		homingTarget = nullptr;
		bossHomingTarget.reset();
		AcquireHomingTarget();
	...
	const float turnSpeedRadians =
		GetAssets().GetGameplayData().GetPickups().homingTurnSpeedDegrees * std::numbers::pi_v<float> / 180.f;
		...
		assets.GetGameplayData().GetProjectile(projectileKind), VisualKind::Enemy)
{
	...
	GetWorld().Effects().Add({ Rendering::EffectEventType::EnemyMuzzleFlash, position, direction });
	...
	, homingTarget(target)
{
	...
	const sf::Vector2f toTarget{ target != nullptr ? target->GetPosition() - position : sf::Vector2f{ 1.f, 0.f } };
	const float angle = std::atan2(toTarget.y, toTarget.x);
	...
	GetWorld().Effects().Add({
		Rendering::EffectEventType::PlayerMuzzleFlash,
		position,
...
void HelperShot::UpdateHoming(float deltaTime)
{
	...
	const float angleDifference =
		std::atan2(std::sin(targetAngle - currentAngle), std::cos(targetAngle - currentAngle));
	const float turnSpeedRadians =
		GetAssets().GetGameplayData().GetPickups().helperBotTurnSpeedDegrees * std::numbers::pi_v<float> / 180.f;
