...
namespace Rendering
{
	namespace
	{
		...
		// GetPulse() breathes between these two intensities rather than between
		// 0 and 1, so the glow never fully switches off at the bottom of the cycle.
		constexpr float PulseMinimumIntensity = 0.58f;
		constexpr float PulseIntensityRange = 0.42f;
		...
		const sf::BlendMode PureAdditive(sf::BlendMode::Factor::One, sf::BlendMode::Factor::One, sf::BlendMode::Equation::Add);
	}
	...
	void NeonGlow::DrawBloom(sf::RenderTarget& target, const sf::FloatRect& bounds, const SourceRenderer& renderSource,
		sf::Color color, bool isPulsing)
	{
		if (needToRebuild || cachedContentSize != bounds.size)
			Rebuild(bounds, renderSource);

		if (needToRebuild || outerBlur.getSize().x == 0u || outerBlur.getSize().y == 0u)
			return;
	...
	void NeonGlow::DrawHighlight(sf::RenderTarget& target, const sf::FloatRect& bounds, sf::Color color) const
	{
		if (needToRebuild || emissive.getSize().x == 0u || emissive.getSize().y == 0u)
			return;
	...
	void NeonGlow::ApplyBlur(const sf::Texture& input, sf::RenderTexture& output, float radius, unsigned int iterations)
	{
	...
	{
		return PulseMinimumIntensity + PulseIntensityRange * (std::sin(elapsedTime * PulseSpeed) * 0.5f + 0.5f);
	}
}